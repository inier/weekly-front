...
export default class Detail extends Component {
  constructor(props) {
    ...
    getDate = () => {
      ...
      request.get(`/api/weeks/list?count=${this.props.match.params.week}`).then((res) => {
    ...
    renderDetail = (weeklyDetail) => {
      ...
      return weeklyDetail.map(card => (
        <li key={card.id} className="item">
          ...
          <blockquote className="detail-item-content">{card.description}</blockquote>
        </li>
      ));
    };
    ...
    renderHistoryIndex = (homeList) => {
      ...
      return reverseList.map(card => (
        <li key={card.count}>
          <Link to={`/detail/${card.count}`} target="_blank" rel="noopener noreferrer">
            <h4>{card.title}</h4>
          </Link>
        </li>
      ));
    ...
    renderTags = (tags) => {
      ...
      return tags.map(tag => (
        <li key={tag.name}>
          <Link to={`/detail/${tag.name}`} target="_blank" rel="noopener noreferrer">
            <h4>{tag.name}</h4>
          </Link>
        </li>
      ));
